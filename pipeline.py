...
class ModelPipeline:
    ...
    def cross_val_metrics(self, model, X, y, model_name):
        ...
        for metric in train_metrics.keys():
            self.fold_stats['Model'].append(model_name)
            self.fold_stats['Dataset'].append('Train')
            self.fold_stats['Metric'].append(metric)
            self.fold_stats['Mean'].append(np.mean(train_metrics[metric]))
            self.fold_stats['Std'].append(np.std(train_metrics[metric]))
        for metric in val_metrics.keys():
            self.fold_stats['Model'].append(model_name)
            self.fold_stats['Dataset'].append('Validation')
            self.fold_stats['Metric'].append(metric)
            self.fold_stats['Mean'].append(np.mean(val_metrics[metric]))
            self.fold_stats['Std'].append(np.std(val_metrics[metric]))
    ...
    def train_and_evaluate(self, X, y):
        ...
        fold_stats_df = pd.DataFrame(self.fold_stats)
        ...
        with pd.ExcelWriter(cv_csv_path.replace(".csv", ".xlsx")) as writer:
            ...
            fold_stats_df.to_excel(writer, sheet_name="FoldStats", index=False)

        cv_metrics_df.to_csv(cv_csv_path, index=False)

        fold_stats_df.to_csv(os.path.join(self.output_dir, '10_fold_crossvalidation_metrics_fold_stats.csv'), index=False)
        ...
        print(f"10-fold crossvalidation fold stats (averages): "
              f"{os.path.join(self.output_dir, '10_fold_crossvalidation_metrics_fold_stats.csv')}")
        return results_df
